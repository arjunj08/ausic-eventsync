...
export default function Meetings() {
  ...
  // Real-time synchronization of agenda lists, minutes, and status updates
  useEffect(() => {
    if (socket && selectedMeeting) {
      ...
      const minutesChannel = `meeting_${selectedMeeting._id}_minutes_update`;
      const statusChannel = `meeting_${selectedMeeting._id}_status_update`;
      ...
      const handleMinutesUpdate = (data) => {
        setSelectedMeeting(prev => {
          if (!prev || prev._id !== selectedMeeting._id) return prev;
          return { ...prev, minutes: data.minutes };
        });
      };

      const handleStatusUpdate = (updatedMeeting) => {
        setSelectedMeeting(prev => {
          if (!prev || prev._id !== selectedMeeting._id) return prev;
