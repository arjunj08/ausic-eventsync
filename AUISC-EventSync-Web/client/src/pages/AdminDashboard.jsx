...
export default function AdminDashboard() {
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      ...
      // Refresh dashboard stats and tasks
      fetchDashboardData();
    } catch (err) {
      ...
      <div className="border-b border-gray-800 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={() => {
            setTaskTitle('');
            setTaskDescription('');
            setTaskTeamId(teams[0]?._id || teams[0]?.id || '');
            setTaskEventId(events[0]?._id || events[0]?.id || '');
            setTaskAssignedTo('');
            setTaskDueDate('');
            setShowTaskModal(true);
          }}
          className="bg-gradient-to-r from-[#00BFFF] to-[#8F5CFF] text-[#0b0c10] font-extrabold px-5 py-3 rounded-lg hover:opacity-90 transition-all flex items-center gap-1.5 cursor-pointer shrink-0"
        >
          ...
          <div className="bg-[#111111] border border-gray-850 max-w-lg w-full rounded-xl p-6 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#00BFFF] hover:bg-[#00BFFF]/80 text-[#0b0c10] font-extrabold py-3.5 rounded-lg transition-all flex items-center justify-center gap-1.5 mt-4"
              >
                {taskLoading ? (
                  <>
                    <span className="animate-spin border-2 border-t-transparent border-black rounded-full h-4 w-4 mr-2"></span>
